...
: pool(p_l, p_r1, p_r2), time(0), ball(p_y, p_theta, p_vx){
    p_y>-p_r1 && p_y<p_r1 ? std::cout << "Ball created at position: " << p_theta << ", " << p_vx << ", " << p_y << std::endl : throw std::invalid_argument("y must be greater than -r1 and less than r1");
...
: ball(p_ball), pool(p_pool), time(0) {
    std::cout << "System created with ball at position: " << ball.getPos()[0] << ", " << ball.getPos()[1] << std::endl;
}
...
template <typename T>
Ball<T>& System<T>::getBall() {
    return ball;
}

template <typename T>
Pool<T>& System<T>::getPool() {
    return pool;
}
...
template <typename T>
std::vector<Collision<T>>& System<T>::getCollisions() {
    return collisions;
}
...
return collisions[collisions.size()].getPos()[1]+(ball.getVel()[1]/ball.getVel()[0])*(pool.getL()-collisions[collisions.size()].getPos()[0]);
}
