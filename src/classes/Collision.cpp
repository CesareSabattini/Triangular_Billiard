...
: pos({p_x, p_y}), vel(
    ...
    p_vx>=0 ? std::cout << "Collision created with velocity: " << p_vx << ", " << p_vy << std::endl : throw std::invalid_argument("vx must be greater than or equal to 0");
...
template <typename T>
std::array<T,2>& Collision<T>::getPos() {
    return pos;
}

template <typename T>
std::array<T,2>& Collision<T>::getVel() {
    return vel;
}

template <typename T>
void Collision<T>::setVel(const std::array<T,2>& p_vel) {
    vel = p_vel;
}
