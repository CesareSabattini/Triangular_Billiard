...
template <typename T> void Analyzer<T>::analyze() {
    results.outputSize = static_cast<int>(outputs.size());
    results.meanY = meanY();
...
template <typename T> T Analyzer<T>::skewnessY() {
    ...
    T skewness = std::accumulate(outputs.begin(), outputs.end(), T(0),
...
template <typename T> T Analyzer<T>::kurtosisY() {
    ...
    T kurtosis = std::accumulate(outputs.begin(), outputs.end(), T(0),
                                 [mean, std_dev](const T &acc,
                                                 const std::array<T, 2> &elem) {
                                     T z = (elem[0] - mean) / std_dev;
                                     return acc + z * z * z * z;
